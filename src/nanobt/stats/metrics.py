...
class Ret(Metric):
    ...
    def __init__(self, name: str | None = None, book_size: float | None = None):
...
class AnnualRet(Ret):
    ...
    def __init__(self, name: str | None = None, book_size: float | None = None, trading_days_per_year: float = 252):
        super().__init__(
...
class SR(Metric):
    ...
    def __init__(self, name: str | None = None, trading_days_per_year: float = 252):
    ...
    def compute(self, df: pl.DataFrame, context: Dict[str, Any]) -> Mapping[str, Any]:
        ...
        with np.errstate(divide='ignore'):
            return {self.name: np.divide(pnl.drop_nans().mean(), pnl.drop_nans().std()) * np.sqrt(c)} # type: ignore
...
class Sortino(Metric):
    ...
    def compute(self, df: pl.DataFrame, context: Dict[str, Any]) -> Mapping[str, Any]:
        ...
        with np.errstate(divide='ignore'):
            return {self.name: np.divide(pnl.drop_nans().mean(), dr) * np.sqrt(c)} # type: ignore
...
class MaxDrawdown(Metric):
    ...
    def compute(self, df: pl.DataFrame, context: Dict[str, Any]) -> Mapping[str, Any]:
        ...
        return {self.name: abs(dd.min())} # type: ignore
...
class MaxLeverage(Metric):
    ...
    def __init__(self, name: str | None = None, book_size: float = 0.0):
        if book_size <= 0.0:
    ...
    def compute(self, df: pl.DataFrame, context: Dict[str, Any]) -> Mapping[str, Any]:
        return {self.name: (df['position'].abs() * df['price']).max() / self.capital} # type: ignore
