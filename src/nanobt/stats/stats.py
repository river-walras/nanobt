...
class Record(ABC):
    ...
    def stats(
        ...
        **Examples**
        ...
        """
        if metrics is None:
            metrics = Record.DEFAULT_METRICS # type: ignore

        if not isinstance(self.df['timestamp'].dtype, pl.Datetime):
            self.df = self.df.with_columns(
                pl.from_epoch('timestamp', time_unit=self._time_unit) # type: ignore
            )
        ...
        stats = [compute_metrics(df, metrics, kwargs) for df in splits] # type: ignore
        # For the entire period.
        stats.append(compute_metrics(self.df, metrics, kwargs)) # type: ignore
